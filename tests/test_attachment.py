...
def test_attach_to_staticmethod(builder_class: Type[DynamicCodeBuilder]) -> None:
    class MyClass:
        @staticmethod
        @builder_class()
        def im_a_staticmethod(a: str, b: int, c: DType) -> Tuple[str, int, DType]:
            return a, b, c
    ...
    assert MyClass().im_a_staticmethod("a", 2, {3: 3}) == ("a", 2, {3: 3})
    # When pytest runs with mypy, this winds up amounting to a type assertion
    _1: "UnboundDynamicCodeRunner[str, [ int, DType], Tuple[str, int, DType]]" = MyClass.im_a_staticmethod  # noqa: F841
    _2: "Callable[[str, int, DType], Tuple[str, int, DType]]" = MyClass.im_a_staticmethod.__call__  # noqa: F841
...
def test_attach_to_classmethod(builder_class: Type[DynamicCodeBuilder]) -> None:
    ...
    _2: "Callable[[str, int, DType], Tuple[str, int, DType]]" = MyClass.im_a_classmethod.__call__  # noqa: F841
